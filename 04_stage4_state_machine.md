...
**测试目标：** 宿主机无法主动连接容器，但容器可以主动连接宿主机并顺利收到回包。

**1. 验证外部无法主动进入：**
在容器 `nft-lab` 内启动一个服务模拟（即使开了服务，外面也进不来）：
```bash
nc -l -p 8080 &
```
在宿主机发起主动访问探测：
...
在宿主机（注意：这次是宿主机做服务端）开启监听：
```bash
# 宿主机上执行
nc -l -p 9090
```
进入容器 `nft-lab`，主动连接宿主机上面的 9090 端口：
```bash
# 容器内执行（请将 172.17.0.1 替换为你实际的宿主机 bridge IP，通常为网关 IP）
nc -vz 172.17.0.1 9090
# 预期结果：succeeded! 
```

**原理解释：**
- 容器发出探测包（SYN），命中 `output` 链的 `policy accept` 放归，**同时状态机记录这个连接的状态**。
- 宿主机回复确认包（SYN+ACK）。包到达容器的 `input` 链，命中第一条规则：`ct state established,related accept`！回包顺利放行。
- 这样，不需要开启任何高位端口，容器就完成了网络通信。通过 `nc` 实测，状态机的生命周期展现得淋漓尽致。
